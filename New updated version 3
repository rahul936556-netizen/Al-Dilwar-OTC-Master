...
<div class="card" id="dashboardBox" style="display:none;">
  ...
  <select id="market">
    ...
    <option>EUR/SGD OTC</option><option>GBP/SGD OTC</option><option>AUD/SGD OTC</option>
    <option>NZD/SGD OTC</option><option>CAD/SGD OTC</option><option>CHF/SGD OTC</option>
    <option>EUR/ZAR OTC</option><option>GBP/ZAR OTC</option><option>AUD/ZAR OTC</option>
    <option>NZD/ZAR OTC</option><option>CAD/ZAR OTC</option><option>CHF/ZAR OTC</option>
    <option>EUR/TRY OTC</option><option>GBP/TRY OTC</option><option>USD/TRY OTC</option>
    <option>AUD/TRY OTC</option><option>NZD/TRY OTC</option><option>CAD/TRY OTC</option>
    <option>CHF/TRY OTC</option><option>EUR/MXN OTC</option><option>USD/MXN OTC</option>
  </select>

  <!-- Timeframe -->
  <select id="timeframe" onchange="resetAutoRefresh()">
...
const indicatorWeights = {RSI:2, MACD:3, EMA:1, SMA:1, Bollinger:2, Stochastic:1, ADX:2, CCI:1};
let tradeHistory=[];

// Generate Signal with weighted & trend logic
...
function generateSignal(){
  ...
  if(!market || !timeframe){ alert("Select Market and Timeframe!"); isGenerating=false; return; }

  // Weighted indicators with profit bias (~65% chance)
  let score=0;
  indicators.forEach(ind=>{
    let sig = Math.random()<0.65 ? 1 : -1; // BUY=1, SELL=-1
    score += sig * indicatorWeights[ind];
  });

  // Add last 5 trades trend effect
  if(tradeHistory.length>0){
    let trendScore=tradeHistory.slice(-5).reduce((acc,t)=>{
      return acc + (t==="BUY 💚"?1:(t==="SELL 🔴"?-1:0));
    },0);
    score += trendScore;
  }
  ...
  if(score>=6) finalSignal="BUY 💚";
  else if(score<=-6) finalSignal="SELL 🔴";

  // Add to trade history
  tradeHistory.push(finalSignal);
  if(tradeHistory.length>50) tradeHistory.shift();

  // Profit estimate
  const profitPercent=Math.floor(Math.random()*30)+65;
...
function getTimeframeMs(){
  ...
  switch(tf){
    case "30 Second": return 30000;
    case "1 Minute": return 60000;
    case "2 Minute": return 120000;
    case "3 Minute": return 180000;
    case "4 Minute": return 240000;
    case "5 Minute": return 300000;
    default: return 60000;
  }
}
function startAutoRefresh(){
  clearInterval(autoRefreshInterval);
  autoRefreshInterval=setInterval(generateSignal,getTimeframeMs());
}
